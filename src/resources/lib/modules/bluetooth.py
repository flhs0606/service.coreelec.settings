...
class bluetooth(modules.Module):
            ...
            if hasattr(self, 'dbusBluezAdapter') and self.dbusBluezAdapter is not None:
                def _unregister():
                    try:
                        ...
                        pass
                t = threading.Thread(target=_unregister)
                t.daemon = True
                t.start()
                t.join(timeout=2)
        except Exception:
    ...
    def find_list_item(self, control_list, entry):
        for i in range(control_list.size()):
            list_item = control_list.getListItem(i)
            if list_item.getProperty('entry') == entry:
                return list_item
        return None

    def remove_duplicate_list_items(self, control_list):
        seen = set()
        i = 0
        while i < control_list.size():
            entry = control_list.getListItem(i).getProperty('entry')
            if entry in seen:
    ...
    def _discover_devices(self):
        ...
        for dbusDevice, device_properties in self.dbusDevices.items():
            ...
            if 'RSSI' in device_properties:
                dictProperties['Strength'] = str(rssi_to_percentage(int(device_properties['RSSI'])))
            ...
            if dbusDevice in new_devices and self.find_list_item(control_list, dbusDevice) is None:
                ...
                if dbusDevice not in self.listItems:
                    existing_item = self.find_list_item(control_list, dbusDevice)
                    if existing_item is not None:
                        self.listItems[dbusDevice] = existing_item
            ...
            self.remove_duplicate_list_items(control_list)
            for dbusDevice in deactivated_devices:
                i = control_list.size() - 1
                while i >= 0:
                    if control_list.getListItem(i).getProperty('entry') == dbusDevice:
                        control_list.removeItem(i)
                    i -= 1
                self.listItems.pop(dbusDevice, None)
...
class Bluez_Listener(dbus_bluez.Listener):
    ...
    def __init__(self, parent):
        ...
        self._last_connect_event = {}
        self._last_disconnect_event = {}
    ...
    def on_properties_changed(self, interface, changed, invalidated, path):
        ...
        if self.parent.visible:
            ...
            if path in self.parent.listItems:
                ...
                if 'RSSI' in changed:
                    self.parent.listItems[path].setProperty('Strength', str(rssi_to_percentage(int(changed['RSSI']))))
            else:
                self.parent.discover_devices()
